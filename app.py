...
pending_codes: dict[str, dict] = {}  # code -> {chat_id, username, first_name}
pending_cookies_setup: dict[int, float] = {}  # chat_id -> created_at timestamp
...
def build_quality_options(info):
    # Keep only the best bitrate format per resolution for a cleaner menu.
    best_by_height = {}
...
async def telegram_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code = secrets.token_hex(3).upper()  # 6 hex chars, e.g. A3F9C2
    pending_codes[code] = {
        "chat_id": chat_id,
...
async def telegram_send_download(chat_id, bot, url, title, format_choice, format_id=None):
    ...
    try:
        ...
        if size_mb > 49:
            ...
            # Do not delete path here; it will be deleted after the HTTP download.
            return
